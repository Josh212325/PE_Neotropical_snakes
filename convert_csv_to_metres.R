...
#
# ###### EDIT THESE ###########################
input_csv_file <- "/Users/josue/Dropbox/1Doutorado/Chapter_2/biodiverse_pipeline-master/snakes_gridded_dist.csv" # input file, csv with columns longitude, latitude in decimal dgrees
long_lat_columns <- c("x", "y")# the names of the longitude and latitude columns go here
###############################################
...
#newproj <- paste0('behrmannCRS') # Berhman
...
convert_to_metres <- spTransform(locs_sub, CRS=behrmannCRS)  #spTransform makes the projection
